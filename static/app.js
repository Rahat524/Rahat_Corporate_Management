...
async function loadControlTower(){if(!$('ftSignals'))return;const x=await api('/api/accounts/control-tower');$('ftCollections').textContent=money(x.overdue_collections_amount);$('ftPayments').textContent=money(x.overdue_payments_amount);$('ftTax').textContent=money(x.pending_tax_amount);$('ftApprovals').textContent=x.pending_approvals;$('ftSignals').innerHTML=[['Overdue collection promises',x.overdue_collections_count],['Overdue scheduled payments',x.overdue_payments_count],['Pending tax items',x.pending_tax_count],['Pending approvals',x.pending_approvals]].map(v=>`<div class="finance-signal ${Number(v[1])?'warning':'ok'}"><b>${esc(v[0])}</b><span>${v[1]}</span></div>`).join('')}
async function loadCollections(){if(!$('crBody'))return;const rows=await api('/api/accounts/collections');$('crBody').innerHTML=rows.map(x=>`<tr><td>${esc(x.promised_date)}</td><td>${x.days_to_promise}</td><td>${esc((x.customer_code||'')+' '+x.customer_name)}</td><td>${esc(x.reference_no||'')}</td><td>${money(x.outstanding_amount)}</td><td>${money(x.promised_amount)}</td><td>${esc(x.owner||'')}</td><td>${esc(x.status)}</td><td>${x.status==='Collected'?'✓':`<button onclick="setCollectionStatus(${x.id},'Collected')">Collected</button>`}</td></tr>`).join('')||'<tr><td colspan="9">No collection promises</td></tr>'}
async function saveCollection(){try{await api('/api/accounts/collections',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({customer_code:$('crCode').value,customer_name:$('crName').value,reference_no:$('crRef').value,outstanding_amount:$('crOutstanding').value,promised_date:$('crDate').value,promised_amount:$('crAmount').value,owner:$('crOwner').value})});loadCollections()}catch(e){alert(e.message)}}
...
let lastUniversalRows=[];
async function loadUniversalFinance(){const p=$('ujPeriod')?.value||new Date().toISOString().slice(0,7);if($('ujPeriod')&&!$('ujPeriod').value)$('ujPeriod').value=p;await Promise.all([loadUniversalJournal(),loadRecurringJournals(),loadCloseOrchestrator()]);}
async function loadUniversalJournal(){if(!$('ujBody'))return;const period=$('ujPeriod')?.value||new Date().toISOString().slice(0,7),source=$('ujSource')?.value||'ALL',q=$('ujSearch')?.value||'';try{const d=await api(`/api/accounts/universal-journal?period=${encodeURIComponent(period)}&source=${encodeURIComponent(source)}&q=${encodeURIComponent(q)}`);lastUniversalRows=d.rows;$('ujDebit').textContent=money(d.total_debit);$('ujCredit').textContent=money(d.total_credit);$('ujCount').textContent=d.rows.length;$('ujBody').innerHTML=d.rows.map(x=>`<tr><td>${esc(x.posting_date)}</td><td><span class="status-chip matched">${esc(x.source)}</span></td><td><b>${esc(x.document_no)}</b></td><td>${esc(x.account)}</td><td>${esc(x.description)}</td><td>${money(x.debit)}</td><td>${money(x.credit)}</td><td>${esc(x.cost_center||'ALL')}</td></tr>`).join('')||'<tr><td colspan="8">No journal rows found.</td></tr>'}catch(e){alert('Universal Journal load failed: '+e.message)}}
function downloadUniversalJournal(){const rows=[['Date','Source','Document','Account','Description','Debit','Credit','Cost Center'],...lastUniversalRows.map(x=>[x.posting_date,x.source,x.document_no,x.account,x.description,x.debit,x.credit,x.cost_center])];const csv=rows.map(r=>r.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download=`Universal_Journal_${$('ujPeriod')?.value||'period'}.csv`;a.click();URL.revokeObjectURL(a.href)}
async function loadRecurringJournals(){if(!$('rjBody'))return;try{const rows=await api('/api/accounts/recurring-journals');$('rjBody').innerHTML=rows.map(x=>`<tr><td><b>${esc(x.template_name)}</b></td><td>${esc(x.debit_account)}</td><td>${esc(x.credit_account)}</td><td>${money(x.amount)}</td><td>${esc(x.last_posted||'Never')}</td><td><button onclick="postRecurringJournal(${x.id})">Post</button></td></tr>`).join('')||'<tr><td colspan="6">No templates configured.</td></tr>'}catch(e){alert(e.message)}}
async function saveRecurringJournal(){const body={template_name:$('rjName').value.trim(),debit_account:$('rjDebitAccount').value.trim(),credit_account:$('rjCreditAccount').value.trim(),amount:Number($('rjAmount').value||0),cost_center:$('rjCostCenter').value.trim()||'ALL',narration:$('rjNarration').value.trim()};if(!body.template_name||!body.debit_account||!body.credit_account||body.amount<=0){alert('Template, both accounts and positive amount required.');return}try{await api('/api/accounts/recurring-journals',{method:'POST',body:JSON.stringify(body)});['rjName','rjDebitAccount','rjCreditAccount','rjAmount','rjCostCenter','rjNarration'].forEach(id=>$(id).value='');await loadRecurringJournals();alert('Recurring journal template saved.')}catch(e){alert(e.message)}}
async function postRecurringJournal(id){const period=$('ujPeriod')?.value||new Date().toISOString().slice(0,7),posting_date=prompt('Posting date (YYYY-MM-DD)',period+'-01');if(!posting_date)return;try{const d=await api(`/api/accounts/recurring-journals/${id}/post`,{method:'POST',body:JSON.stringify({posting_date})});alert('Posted voucher '+d.voucher_no);await loadUniversalFinance()}catch(e){alert(e.message)}}
async function loadCloseOrchestrator(){if(!$('closeChecks'))return;const period=$('ujPeriod')?.value||new Date().toISOString().slice(0,7);try{const d=await api('/api/accounts/close-orchestrator?period='+encodeURIComponent(period));$('ujCloseStatus').textContent=d.status;$('ujBlockers').textContent=d.blockers+' blocker(s)';$('closeChecks').innerHTML=d.checks.map(x=>`<div class="s4-check ${x.ok?'ok':'warning'}"><b>${x.ok?'✓':'!'} ${esc(x.name)}</b><span>${esc(x.detail)}</span></div>`).join('')}catch(e){$('closeChecks').innerHTML=`<div class="s4-check warning"><b>Close check failed</b><span>${esc(e.message)}</span></div>`}}
document.addEventListener('click',e=>{if(e.target.closest('.nav')?.dataset.page==='universalFinance')setTimeout(loadUniversalFinance,80)});

// V49 Excel Auto Sync & Integration Center
async function loadExcelSyncCenter(){
 if(!$('xsSourceBody'))return;
 try{
  const [sources,history,rows]=await Promise.all([api('/api/accounts/sync-sources'),api('/api/accounts/sync-history'),api('/api/accounts/sync-rows?limit=100')]);
  $('xsSources').textContent=sources.length;$('xsLast').textContent=sources.find(x=>x.last_sync_at)?.last_sync_at?.replace('T',' ')||'Never';$('xsRows').textContent=sources.reduce((a,x)=>a+Number(x.last_row_count||0),0);
  $('xsSourceBody').innerHTML=sources.map(x=>`<tr><td><b>${esc(x.source_name)}</b></td><td>${esc(x.store_code)}</td><td>${esc(x.target_module)}</td><td>${esc((x.last_sync_at||'Never').replace('T',' '))}</td><td>${esc(x.last_file_name||'-')}</td><td>${x.last_row_count||0}</td><td><span class="status-chip ${x.is_active?'matched':'pending'}">${x.is_active?'Active':'Inactive'}</span></td><td><button onclick="toggleSyncSource(${x.id})">${x.is_active?'Pause':'Activate'}</button> <button onclick="rotateSyncKey(${x.id},'${esc(x.source_name).replaceAll("'","&#39;")}')">Rotate Key</button> <button onclick="selectSyncSource(${x.id})">Map</button></td></tr>`).join('')||'<tr><td colspan="8">No sync source configured.</td></tr>';
  $('xsHistoryBody').innerHTML=history.map(x=>`<tr><td>${esc((x.synced_at||'').replace('T',' '))}</td><td>${esc(x.source_name)}</td><td>${esc(x.store_code)}</td><td>${esc(x.file_name||'')}</td><td>${x.row_count||0}</td><td>${x.inserted_count||0}</td><td>${x.skipped_count||0}</td><td><span class="status-chip ${x.status==='Success'?'matched':'pending'}">${esc(x.status||'')}</span></td></tr>`).join('')||'<tr><td colspan="8">No sync history yet.</td></tr>';
...
// V50 Integration Governance, Mapping & Validation
let syncSourcesCache=[];
async function loadSyncGovernance(){
 try{const d=await api('/api/accounts/sync-health');$('xgScore').textContent=d.score+'%';$('xgStale').textContent=d.stale_sources;$('xgIssues').textContent=d.validation_issues;const b=$('xgStatus');b.className='s4-check '+(d.status==='Healthy'?'ok':'warning');b.innerHTML=`<b>${esc(d.status)} Integration Health</b><span>${d.active_sources}/${d.total_sources} active sources · ${d.failed_batches} failed batches · ${d.stale_sources} stale sources</span>`;}catch(e){alert(e.message)}
}
async function loadSyncMappings(){
 if(!$('xmBody'))return;try{syncSourcesCache=await api('/api/accounts/sync-sources');$('xmSource').innerHTML=syncSourcesCache.map(x=>`<option value="${x.id}">${esc(x.source_name)} — ${esc(x.store_code)}</option>`).join('');const [maps,logs]=await Promise.all([api('/api/accounts/sync-mappings'),api('/api/accounts/sync-validation-log')]);$('xmBody').innerHTML=maps.map(x=>`<tr><td>${esc(x.source_name)}</td><td><b>${esc(x.target_field)}</b></td><td>${esc(x.source_column)}</td><td>${x.is_required?'Yes':'No'}</td><td>${esc(x.default_value||'')}</td></tr>`).join('')||'<tr><td colspan="5">No field mappings.</td></tr>';$('xvBody').innerHTML=logs.map(x=>`<tr><td>${esc((x.created_at||'').replace('T',' '))}</td><td>${esc(x.source_name)}</td><td><span class="status-chip ${x.severity==='Error'||x.severity==='Critical'?'pending':'matched'}">${esc(x.severity)}</span></td><td>${esc(x.field_name||'-')}</td><td>${esc(x.message)}</td></tr>`).join('')||'<tr><td colspan="5">No validation issues.</td></tr>';}catch(e){alert(e.message)}
}
async function toggleSyncSource(id){try{await api(`/api/accounts/sync-sources/${id}/toggle`,{method:'POST'});await loadExcelSyncCenter()}catch(e){alert(e.message)}}
async function rotateSyncKey(id,name){if(!confirm('Rotate API key for '+name+'? Old agent key will stop working.'))return;try{const d=await api(`/api/accounts/sync-sources/${id}/rotate-key`,{method:'POST'});const b=$('xsKeyBox');b.style.display='block';b.className='s4-check ok';b.innerHTML=`<b>New API Key — Copy Now</b><span><code style="user-select:all">${esc(d.api_key)}</code><br>Replace old key in Sync Agent.</span>`;}catch(e){alert(e.message)}}
function selectSyncSource(id){$('xmSource').value=String(id);$('xmTarget').focus()}
async function saveSyncMapping(){const body={source_id:Number($('xmSource').value),target_field:$('xmTarget').value.trim(),source_column:$('xmColumn').value.trim(),default_value:$('xmDefault').value.trim(),is_required:$('xmRequired').checked};if(!body.source_id||!body.target_field||!body.source_column){alert('Source, target field and Excel column required.');return}try{await api('/api/accounts/sync-mappings',{method:'POST',body:JSON.stringify(body)});$('xmTarget').value='';$('xmColumn').value='';$('xmDefault').value='';$('xmRequired').checked=false;await loadSyncMappings()}catch(e){alert(e.message)}}
async function validateSyncSource(){const id=Number($('xmSource').value);if(!id)return;try{const d=await api(`/api/accounts/sync-sources/${id}/validate`,{method:'POST'});alert(`${d.status}: ${d.rows_checked} rows checked, ${d.errors} errors, ${d.warnings} warnings`);await Promise.all([loadSyncMappings(),loadSyncGovernance()])}catch(e){alert(e.message)}}
const _oldLoadExcelSyncCenter=loadExcelSyncCenter;loadExcelSyncCenter=async function(){await _oldLoadExcelSyncCenter();await Promise.all([loadSyncGovernance(),loadSyncMappings()]);};

// V51 Enterprise Integration Control
async function loadIntegrationControl(){
 if(!$('icBody'))return;
 try{const [d,sources]=await Promise.all([api('/api/accounts/integration-control'),api('/api/accounts/sync-sources')]);$('icSources').textContent=d.active_sources;$('icPending').textContent=d.pending;$('icApproved').textContent=d.approved;$('icRejected').textContent=d.rejected;$('icSource').innerHTML=sources.map(x=>`<option value="${x.id}">${esc(x.source_name)} — ${esc(x.store_code)}</option>`).join('');await loadIntegrationQueue()}catch(e){alert(e.message)}
}
async function loadIntegrationQueue(){if(!$('icBody'))return;try{const rows=await api('/api/accounts/integration-queue?status='+encodeURIComponent($('icStatus').value));$('icBody').innerHTML=rows.map(x=>`<tr><td>${x.id}</td><td>${esc(x.source_name)}</td><td>${esc(x.store_code)}</td><td>${esc(x.row_date||'-')}</td><td>${esc(x.target_module||'')}</td><td><small>${esc(JSON.stringify(x.payload)).slice(0,180)}</small></td><td><span class="status-chip ${x.status==='Approved'?'matched':'pending'}">${esc(x.status)}</span></td><td>${x.status==='Pending'?`<button onclick="integrationDecision(${x.id},'Approved')">Approve</button> <button onclick="integrationDecision(${x.id},'Rejected')">Reject</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="8">No integration queue rows.</td></tr>'}catch(e){alert(e.message)}}
...
window.login = async function () {
    ...
    const button = document.querySelector("#loginScreen .login-card button");
    ...
    try {
        ...
        document.querySelectorAll(
            ".loading-overlay,.loader-overlay,.page-loader,#loadingScreen,#loader"
        ).forEach(el => {
            el.style.display = "none";
            el.remove();
        });

        /* Dashboard initialization background mein chalegi */
        Promise.race([
        ...
        if (button) {
            ...
            button.textContent = oldText || "Login to Dashboard";
        }
    }
};
