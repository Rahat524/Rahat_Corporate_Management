...
function clearCorporateEntry(){["corpEntryDoc","corpEntryCode","corpEntryName","corpEntryDebit","corpEntryCredit"].forEach(id=>{if($(id))$(id).value=""});if($("corpEntryDate"))$("corpEntryDate").value=new Date().toISOString().slice(0,10);updateCorporateEntryBalance()}
async function saveCorporateEntry(){const entry={document_number:$("corpEntryDoc").value.trim(),date:$("corpEntryDate").value,code:$("corpEntryCode").value.trim(),name:$("corpEntryName").value.trim(),debit:$("corpEntryDebit").value||0,credit:$("corpEntryCredit").value||0};if(!entry.document_number)return alert("Document No. required.");if(!entry.date)return alert("Date required.");if(!entry.code||!entry.name)return alert("Valid Customer Code required.");const r=await api("/api/customer-ledger/bulk",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({entries:[entry]})});if(r.inserted){alert("Corporate Customer entry saved successfully.");clearCorporateEntry();await Promise.all([loadCorporate(),loadDashboard(),loadDuplicates()])}else alert((r.skipped&&r.skipped[0]&&r.skipped[0].reason)||"Entry save nahi hui.")}
